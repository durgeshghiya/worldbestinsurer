  ...
  return (
    <section className="mt-10 max-w-[760px] mx-auto" aria-label="Editorial analysis">
      <h2 className="text-[18px] font-bold text-text-primary mb-4 tracking-[-0.01em]">
        How {(a.shortName || a.name)} and {(b.shortName || b.name)} actually differ
      </h2>
...
function buildLede(a: Insurer, b: Insurer, country: string): string {
  ...
  let body =
    `${(a.shortName || a.name)} and ${(b.shortName || b.name)} are two of the insurers we track in ${country}. ` +
    `${(a.shortName || a.name)} is headquartered in ${a.headquarters || "an undisclosed location"}` +
    (aYears != null ? ` and has been operating for ${aYears} years (established ${a.established})` : "") +
    `; ${(b.shortName || b.name)} is headquartered in ${b.headquarters || "an undisclosed location"}` +
  ...
  if (aYears && bYears) {
    ...
    if (diff >= 20) {
      ...
      body +=
        `That is roughly ${diff} years of difference in market experience — ${(older.shortName || older.name)} predates ${(younger.shortName || younger.name)} by a generation, which usually translates into more granular underwriting data and a larger book of legacy claims to reference. ` +
...
function buildClaimRecordParagraph(a: Insurer, b: Insurer): string {
  ...
  if (ar && br) {
    ...
    if (!higher || diff < 1) {
      return `Claim settlement ratios are effectively identical: ${ar}% for ${(a.shortName || a.name)}${ay ? ` (${ay})` : ""} and ${br}% for ${(b.shortName || b.name)}${by ? ` (${by})` : ""}. Reliability of claims paid is not a meaningful differentiator between these two — both perform within roughly the same band.`;
    }
    if (diff < 3) {
      return `Claim settlement runs ${ar}% for ${(a.shortName || a.name)}${ay ? ` (${ay})` : ""} versus ${br}% for ${(b.shortName || b.name)}${by ? ` (${by})` : ""}. The ${diff}-point gap is small and may be within year-to-year noise rather than a structural difference; reasonable buyers can weight this either way.`;
    }
    return (
      `On the most consequential metric — does the insurer actually pay claims when filed — ${(higher.shortName || higher.name)} reports a claim settlement ratio of ${higher === a ? ar : br}%` +
  ...
  if (ar) {
    return `${(a.shortName || a.name)} reports a claim settlement ratio of ${ar}%${ay ? ` (${ay})` : ""}. ${(b.shortName || b.name)}'s ratio is not currently disclosed in our data — itself a signal worth weighing. Insurers that publish settlement ratios and let those numbers be audited against complaints data are easier to plan around.`;
  }
  if (br) {
    return `${(b.shortName || b.name)} reports a claim settlement ratio of ${br}%${by ? ` (${by})` : ""}. ${(a.shortName || a.name)}'s ratio is not currently disclosed in our data — itself a signal worth weighing.`;
  }
...
function buildBreadthParagraph(
  ...
  let body =
    `${(a.shortName || a.name)} operates across ${aCats} ${aCats === 1 ? "line" : "lines"} (${a.categories.join(", ")}), with ${pa.length} ${pa.length === 1 ? "product" : "products"} in our database. ` +
    `${(b.shortName || b.name)} operates across ${bCats} ${bCats === 1 ? "line" : "lines"} (${b.categories.join(", ")}), with ${pb.length} ${pb.length === 1 ? "product" : "products"}. `;
...
function buildNetworkParagraph(a: Insurer, b: Insurer): string | null {
  ...
  if (an && bn) {
    ...
    return (
      `On cashless hospital network — the most practically important figure for health insurance — ${(bigger.shortName || bigger.name)} lists ${biggerCount.toLocaleString()} hospitals versus ${smallerCount.toLocaleString()} for the other. ` +
  ...
  if (an) {
    return `${(a.shortName || a.name)} lists a cashless network of ${an.toLocaleString()} hospitals; ${(b.shortName || b.name)}'s network size is not currently disclosed in our data.`;
  }
  if (bn) {
    return `${(b.shortName || b.name)} lists a cashless network of ${bn.toLocaleString()} hospitals; ${(a.shortName || a.name)}'s network size is not currently disclosed in our data.`;
  }
  return null;
}
...
function buildHowToDecideParagraph(a: Insurer, b: Insurer, country: string): string {
  return (
    `How to actually choose between ${(a.shortName || a.name)} and ${(b.shortName || b.name)} in ${country}: the insurer choice is one input, but the specific plan you pick within that insurer matters more. ` +
